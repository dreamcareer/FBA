...
async function runArticlesSync(
  mode: "full" | "diff",
  emit: EmitFn
): Promise<ArticlesSyncResult> {
  // Step 1: 一覧APIで全商品を取得
  emit({ type: "phase", label: "商品一覧を取得中" });
  const summaries = await fetchArticles((current, page) => {
    emit({
      type: "progress",
      current,
      label: `商品一覧 ページ${page} (${current}件取得済み)`,
    });
  });
  console.log(`[sync/articles] Found ${summaries.length} articles (mode=${mode})`);

  // カテゴリを事前作成
  emit({ type: "phase", label: "カテゴリを準備中" });
  ...
  for (let i = 0; i < toFetch.length; i++) {
    ...
    if (!identCode) {
      skipped++;
      emit({ type: "progress", current: i + 1, total: toFetch.length });
      continue;
    }
  ...
  console.log(
    `[sync/articles] Done! Created: ${created}, Updated: ${updated}, Skipped: ${skipped}`
  );
...
export async function POST(req: NextRequest) {
  ...
  const mode = (req.nextUrl.searchParams.get("mode") ?? "diff") as "full" | "diff";

  if (wantsStream(req)) {
    return ndjsonStream(async (emit) => {
      const result = await runArticlesSync(mode, emit);
      return { success: true, ...result };
    });
  }

  try {
    const result = await runArticlesSync(mode, () => {});
