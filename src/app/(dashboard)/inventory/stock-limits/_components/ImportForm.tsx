...
export default function ImportForm() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch("/api/stock-limits/import", {
        method: "POST",
        body: formData,
      });
      ...
      <form onSubmit={onSubmit} className="space-y-3">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">
            以下いずれかのCSVに対応:
            <br />・SKU, 在庫上限（例: 1d10eb750,200）
            <br />・Amazon在庫計画レポート（Child_ASIN と Upper_Limit 列）
          </p>
        </div>
          ...
          {result.success && result.data ? (
            <div className="space-y-2">
              ...
              <ul className="space-y-1 text-xs">
                ...
                <li>
                  更新: {result.data.updated.toLocaleString()} 件
                  {result.data.matchedBy && (
                    <span className="text-gray-500">
                      （{result.data.matchedBy === "asin" ? "Child_ASIN" : "SKU"} で突合）
                    </span>
                  )}
                </li>
                <li>
                  {result.data.matchedBy === "asin" ? "ASIN" : "SKU"}未登録（スキップ）:{" "}
                  {result.data.unmatched.toLocaleString()} 件
                </li>
                {result.data.skippedEmpty > 0 && (
                  <li>在庫上限が空欄（スキップ）: {result.data.skippedEmpty.toLocaleString()} 件</li>
                )}
                {result.data.skippedInvalid > 0 && (
                  <li>在庫上限が数値以外（スキップ）: {result.data.skippedInvalid.toLocaleString()} 件</li>
                )}
              </ul>
